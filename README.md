...
*   **Formateo**: Muestreo/re-muestreo, cuantización (Uniforme/A-law), codificación binaria y scrambling.
...
*   **Features**: Cuantización ajustable (bits, A-law o uniforme) y aleatorización (Scrambling).
...
flowchart LR
    ...
    subgraph L1["Formateo: Fuente Digital"]
        direction TB
        S1["Fuente de información<br/>(Audio/Texto)"] --> S2["Muestreo + Cuantización<br/>(Uniforme o A-law)"]
...
**Formateo:**
```bash
python -m src.main --audio data/voice.wav --n_bits 8 --quantizer alaw --out outputs/cli_lab1
```
